...
def val_ada(data_loader, model, loss_function, global_step, args, writer, loger, epoch=None):
    ...
    for index, (inputs, labels, names) in enumerate(process):
        ...
        with torch.no_grad():
            inputs, labels = inputs.cuda(non_blocking=True), labels.cuda(non_blocking=True)
            outputs, actions = model(inputs)
            action_list.append(actions.cpu().numpy())  # num_act N
        ...
        actions_transport = actions.t()
        for i, x in enumerate(predict):
            all_pre_true.append(str(x) + ',' + str(true[i]) + '\n')
            all_actions.append(str(true[i]) + ',' + str(torch.nonzero(actions_transport[i]).item()) + '\n')
            if x != true[i]:
                wrong_path_pre_ture.append(str(i) + ',' + str(x) + ',' + str(true[i]) + '\n')
    ...
    print_str = model.module.get_policy_usage_str(action_list, label_list)   #actions and gflops all
    ...
    return loss_flops.avg + loss_cls.avg, acces.avg, score_dict, all_pre_true, all_actions, wrong_path_pre_ture, print_str
